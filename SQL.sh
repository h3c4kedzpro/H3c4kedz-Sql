...
do
   ...
   if [ -z "$link" ]
   then
      echo -e "${R}Check the connection or an error occurs"
   fi
...
then
  echo -e "
           ...
           ---------------------\n"
  echo -e "\n${N}${P}Take dork from the database"
  for set in $(seq 22)
  do
...
intro() {
  ...
  echo -e "   USE THESE TOOLS FOR GOOD PURPOSES HELPING TO BE CARELESS\n      NOT TO DAMAGE EVERYTHING TO THE USER BORNE"
  echo -e "            Don't be evil because devil is in your mind\n                 [ Thx to H3c4kedz sec Team ]           \n"

}
adminfin() {
echo -e "${N}[${G}*${N}]\033[2m Search for a Dorking Login page${N}"
pa="1"
trap break INT
...
banner5() {
  ...
  ${G}  //////   ' \/ \`   ---   ${N}    \033[2m |==┣▇▇▇▇▇▇▇▇═─-SQl-i ${N}
${G}   //// / // :    : ---   ${N}       ${R}AGGRESSIV${N}1njector${N}
${G}  // /   /  /\`    '--     ${N}      ${O}./H3c4kedz${O}3${O}n${N} 
${G} //          //..\\${N}
    ${G}    ====UU====UU====
            '//||\\\`
              ''\`\`
